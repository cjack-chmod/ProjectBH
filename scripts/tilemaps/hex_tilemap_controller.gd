...
func _toggle_alt_cell(_pos_clicked : Vector2) -> void:
	# Finds the cell in the atlas and its current alt
		var _current_atlas_coords : Vector2 = get_cell_atlas_coords(_main_layer, _pos_clicked)
		var _current_tile_alt : int = get_cell_alternative_tile(_main_layer, _pos_clicked)

		# Increments the alternative (loop since there can be multiple alts )
		if _current_tile_alt > -1:
			var _number_of_alts_for_clicked : int = tile_set.get_source(_main_atlas_id)\
					.get_alternative_tiles_count(_current_atlas_coords)
			set_cell(_main_layer, _pos_clicked, _main_atlas_id, _current_atlas_coords, 
