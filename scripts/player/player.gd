...
func _move_to_tile_centre(_tile_coords: Vector2, _tween_weight: float) -> void:
	...
	var _new_position: Vector2 = GlobalTileFunctions.find_centre_of_tile(_tile_coords)
...
func _find_and_move_to_adjacent_tile(_direction: GlobalTileFunctions.HEXDIR) -> void:
	...
	var _current_tile: Vector2 = GlobalTileFunctions.find_tile_coordinates(self.global_position)

	# finding adjacent tile based on direction
	var _new_tile: Vector2 = GlobalTileFunctions.find_adjacent_tile(_current_tile, _direction)
